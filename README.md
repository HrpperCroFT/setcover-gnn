...
Существует симметричная квадратная матрица $A \in R^{n \times n}$. Через неё определяется функция $f_A : \{0, 1\}^n \xrightarrow{} R, f_A(x) =x^T A x$. Требуется найти глобальный минимум этой функции.
